...
impl<'ole> super::ole::Reader<'ole> {
    ...
    pub(crate) fn build_directory_entries(&mut self) -> Result<(), super::error::Error> {
        let n_entry_by_sector = self.sec_size.as_ref().unwrap() / super::constants::DIRECTORY_ENTRY_SIZE;
        let mut entries = std::vec::Vec::<Entry>::with_capacity(self.dsat.as_ref().unwrap().len() * n_entry_by_sector);
    ...
    fn get_short_stream_slices(&self, chain: &std::vec::Vec<u32>, size: usize) -> Result<EntrySlice, super::error::Error> {
    ...
    fn get_stream_slices(&self, chain: &std::vec::Vec<u32>, size: usize) -> Result<EntrySlice, super::error::Error> {
